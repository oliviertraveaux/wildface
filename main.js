...
<ul class="logo-svg">
    <li>
    <a class="main-nav-first" href="search.html"> 
        <svg width="30" height="27" viewBox="5 -2 25 30" fill="none">
    ...
    <li>
    <a class="main-nav-first" href="Profil.html"> 
...
<div class="picture-text">
    ...
    <figure class="picture"><img class="imgBigg" src="${i % 2 != 0 ? reponseJS.users[i].picture.large : ""}" alt=""></figure>
</div>
...
`;
    ...
    article.innerHTML += `
                    <figure class="round"><img src="${reponseJS.users[5].picture.thumbnail}"</figure>
                    `
    article.appendChild(lien);
    const submitValide = document.createElement('input');
    submitValide.type = "submit";
    submitValide.value = "✔";
    const button = document.createElement('input');
    button.classList.add('input');
    button.type = "search";
    submitValide.classList.add('formulaire');
    lien.classList.add('dynamic');
    lien.appendChild(button);
    lien.appendChild(submitValide);

    var searchBar = document.querySelector('input[type=search]');
    // button.addEventListener('enter', function (event) {
    lien.addEventListener('change', function (event) {
        event.preventDefault();
        let entrer = searchBar.value;
        for (let i = 0; i < 8; i++) {
            if (entrer[0].toUpperCase() + entrer.slice(1) == reponseJS.users[i].name.first) {
                lien.href = `#${reponseJS.users[i].name.first}`;
                setTimeout(() => {
                    lien.href = "";
                    button.value = "";
                }, 1000);
            } else if (entrer != reponseJS.users[i].name.first) {
